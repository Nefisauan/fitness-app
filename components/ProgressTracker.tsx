...
import { PhysiqueAnalysis, WeeklyCheckin, ProgressPhoto, PhotoAngle } from '@/lib/types';
...
interface ProgressTrackerProps {
  ...
  onSaveCheckin: (checkin: Omit<WeeklyCheckin, 'id' | 'createdAt'>) => Promise<void>;
  latestCheckin: WeeklyCheckin | null;
  // Photo props
  photos: ProgressPhoto[];
  photoDates: string[];
  onUploadPhoto: (file: File, angle: PhotoAngle, date?: string) => Promise<void>;
  onDeletePhoto: (photoId: string, storagePath: string) => Promise<void>;
  getPhotoSignedUrl: (storagePath: string) => Promise<string | null>;
}

type SubTab = 'checkins' | 'photos' | 'history';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-blue-600 rounded-2xl p-6 text-white">
        ...
        <p className="text-indigo-100">Track your improvements over time with check-ins, photos, and assessments.</p>
      </div>

      {/* Sub-tab navigation */}
      <div className="flex gap-1 bg-white/5 backdrop-blur-xl rounded-xl p-1 border border-slate-700/50">
        {subTabs.map((tab) => (
          <button
            key={tab.id}
      ...
      {activeSubTab === 'checkins' && (
        <div className="space-y-6">
          <WeeklyCheckinForm onSave={onSaveCheckin} lastCheckin={latestCheckin} />
