...
function ScoreBar({ score, maxScore = 10, color }: { score: number; maxScore?: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm font-medium text-gray-700 w-8">{score}/{maxScore}</span>
    </div>
  );
}
...
function DevelopmentBadge({ development }: { development: string }) {
  const colors: Record<string, string> = {
    underdeveloped: 'bg-amber-100 text-amber-700',
    balanced: 'bg-green-100 text-green-700',
    well_developed: 'bg-blue-100 text-blue-700',
    overdominant: 'bg-purple-100 text-purple-700',
  };
  ...
  return (
    <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors[development] || 'bg-gray-100 text-gray-700'}`}>
...
function SeverityBadge({ severity }: { severity: string }) {
  const colors: Record<string, string> = {
    ...
    notable: 'bg-red-100 text-red-700',
  };

  return (
    <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors[severity] || 'bg-gray-100 text-gray-700'}`}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">Muscle Development</h3>
                <p className="text-sm text-gray-500">Symmetry & balance assessment</p>
              </div>
            </div>
          ...
          <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
            {analysis.muscle.groups.map((group, index) => (
              <div key={index} className="pb-3 border-b border-gray-100 last:border-0">
                ...
                {group.notes && (
                  <p className="text-xs text-gray-500 mt-1">{group.notes}</p>
                )}
              </div>
            ))}
          </div>
          ...
          {analysis.muscle.priorityAreas.length > 0 && (
            <div className="mt-6 p-4 bg-amber-50 rounded-xl">
              <p className="text-sm font-medium text-amber-800 mb-2">Priority Areas to Develop</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">Postural Indicators</h3>
                <p className="text-sm text-gray-500">Alignment & structural patterns</p>
              </div>
            </div>
            ...
            <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
              {analysis.posture.indicators.map((indicator, index) => (
                <div key={index} className="p-4 bg-gray-50 rounded-xl">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-gray-900">{indicator.area}</span>
                    <SeverityBadge severity={indicator.severity} />
                  </div>
                  <p className="text-sm text-gray-600 mb-2">{indicator.indicator}</p>
                  <div className="flex items-start gap-2">
                    <svg className="w-4 h-4 text-emerald-500 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p className="text-sm text-emerald-700">{indicator.recommendation}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">Movement Quality</h3>
                <p className="text-sm text-gray-500">Mobility, stability & compensation patterns</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="p-4 bg-gray-50 rounded-xl text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">Overall Quality</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {analysis.movement.flags.map((flag, index) => (
                <div key={index} className="p-4 border border-gray-200 rounded-xl">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-gray-900">{flag.movement}</span>
                  ...
                  <p className="text-sm text-gray-600 mb-2">{flag.observation}</p>
                  <p className="text-xs text-gray-500">Affected: {flag.affectedArea}</p>
                  <div className="mt-2 pt-2 border-t border-gray-100">
                    <p className="text-sm text-emerald-700">{flag.recommendation}</p>
                  </div>
                </div>
              ))}
            </div>
